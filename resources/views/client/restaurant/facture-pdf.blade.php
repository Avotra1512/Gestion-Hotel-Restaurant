...
<head>
    <meta charset="UTF-8">
    <title>Facture Commande MISALO #{{ str_pad($commande->id, 6, '0', STR_PAD_LEFT) }}</title>
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .section-title { font-size:11px; text-transform:uppercase; letter-spacing:2px; color:#888; margin-bottom:12px; font-weight:700; border-left:3px solid #f59e0b; padding-left:10px; }
        .badge { display:inline-block; background:#f59e0b; color:#000; padding:4px 14px; border-radius:100px; font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:1px; margin-bottom:20px; }
        ...
        table tbody td { padding:10px 14px; font-size:12px; color:#333; }
        .total-section { display:flex; justify-content:flex-end; margin-bottom:24px; }
        .total-box { background:#1a1a1a; color:#fff; padding:18px 24px; min-width:260px; border-radius:4px; }
        .total-row { display:flex; justify-content:space-between; padding:4px 0; font-size:12px; color:rgba(255,255,255,.6); }
        .total-row.main { border-top:1px solid rgba(255,255,255,.15); padding-top:10px; margin-top:6px; }
        .total-row.main .label { font-size:14px; font-weight:700; color:#fff; }
        .total-row.main .amount { font-size:20px; font-weight:900; color:#f59e0b; }
        .note-box { background:#fffbeb; border:1px solid #fde68a; border-left:4px solid #f59e0b; padding:12px 16px; border-radius:4px; font-size:11px; color:#78350f; margin-bottom:24px; line-height:1.6; }
        .footer { background:#1a1a1a; color:rgba(255,255,255,.4); text-align:center; padding:16px 40px; font-size:10px; letter-spacing:1px; position:fixed; bottom:0; left:0; right:0; }
    </style>
</head>
<body>
...
<div class="info-section">
    ...
    <div class="info-block">
        ...
        <div class="info-detail">{{ $commande->email }}<br>Commande du {{ $commande->created_at->format('d/m/Y à H:i') }}</div>
    </div>
</div>

<div class="content">
    <div class="badge">🍽️ Commande servie</div>
    ...
    <div class="total-section">
        <div class="total-box">
            <div class="total-row"><span>Nombre d'articles</span><span>{{ $commande->items->sum('quantite') }}</span></div>
    ...
    <div class="note-box">
        <strong>Merci</strong> pour votre commande au Restaurant MISALO. Ce document constitue votre reçu officiel.
        À bientôt !
    </div>
</div>

<div class="footer">MISALO — Restaurant · Antananarivo, Madagascar · Commande #{{ str_pad($commande->id, 6, '0', STR_PAD_LEFT) }}</div>
